...
def crossover(parent1, parent2, rate):
    ...
    if np.random.random()>rate:
        ...
        #print "l=",l
        point = np.random.randint(len(parent1)-2)
...
def reproduce(selected, pop_size, no_var, bits_per_param, p_cross):
    ...
    while len(children)<pop_size:
        p1 = selected[np.random.randint(pop_size-1),:]
        p2 = selected[np.random.randint(pop_size-1),:]
        
        #print p1,p2
        
        child=np.zeros((no_var*bits_per_param))
        for i in range(no_var):
            a=p1[i*bits_per_param: (i+1)*bits_per_param]
            b=p2[i*bits_per_param: (i+1)*bits_per_param]
            
            child[i*bits_per_param: (i+1)*bits_per_param] = crossover(a,b , p_cross)
